...
resource "aws_instance" "backend" {
  ...
  user_data = base64encode(templatefile("${path.module}/user_data.sh", {
    ...
    REDIS_CACHE_URL      = var.redis_enabled ? "redis://${aws_elasticache_replication_group.semantic_cache[0].primary_endpoint_address}:${aws_elasticache_replication_group.semantic_cache[0].port}" : ""
  }))
